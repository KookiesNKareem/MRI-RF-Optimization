...
function objective_vectorized(Bx_values)
    ...
    for idx in 1:num_points
        ...
        # Extra penalty for zero targets with non-zero final states
        for component in 1:3
            if abs(target[component]) < 1e-8
                final_val = final_state[component]
                zero_target_penalty += 100.0 * final_val^2
            end
        end
    end
    ...
    total_loss += 1e6 * B1_power_loss + zero_target_penalty
